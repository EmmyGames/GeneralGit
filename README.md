...
#### Serializable
Variables that are Serializable are shown in the Inspector window in Unity.
...
> ##### SCREAMING_CASE
> Words are all upper case and separated by an underscore, e.g. `DESERT_EAGLE`, `STYLE_GUIDE`, `A_SERIES_OF_WORDS`.
...
You must have a good reason to ignore warnings but there is no exception to leaving errors.
...
##### Case
By default, variables use camelCase

`static` variables use PascalCase

`constant` variables and `enum` members use SCREAMING_CASE

Use PascalCase for abbreviations of 4 characters or more rather than fully uppercase (GUI, Guid) unless it is the first word of a pascalCase variable, in which case it is all lowercase (gui, guid).
...
##### Private Variables
Private variables should have a prefix with a underscore `_myVariable` and use the same casing they normally would.
